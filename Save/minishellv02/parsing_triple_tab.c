...
// int	ft_handle_infile(t_list *shell, t_commande *t_cmd,
// 		t_all *all, int i, int *prev_infd);
// int	ft_handle_limiter(t_list *shell, t_commande *t_cmd,
//		t_all *all, int i, int *prev_infd);
// int	ft_handle_outfile(t_list *shell, t_commande *t_cmd,
// 		t_all *all, int i, int *prev_outfd);
// int	ft_handle_append(t_list *shell, t_commande *t_cmd,
//		t_all *all, int i, int *prev_outfd);
// int	ft_handle_pipe(t_commande *t_cmd, int *i,
//		int *prev_infd, int *prev_outfd);
...
int ft_prepare_heredoc_file(t_list *shell, t_commande *t_cmd, t_all *all, int i, int *prev_infd)
{
	if (*prev_infd != -1)
		close(*prev_infd);
	t_cmd->cmd_tab[i].infd = open("temp", O_WRONLY | O_CREAT | O_APPEND, 0644);
...
int ft_handle_limiter(t_list *shell, t_commande *t_cmd, t_all *all, int i, int *prev_infd)
{
	int status;
	int result;

	// Préparer le fichier temporaire
	if (ft_prepare_heredoc_file(shell, t_cmd, all, i, prev_infd) < 0)
		return (0); // Continue parsing même en cas d'erreur
	...
	else 
	{
		...
		// Gérer les signaux potentiels
		result = ft_handle_heredoc_signals(all, status);
		if (result < 0)
			return (-1);
		
		// Traiter la sortie
		ft_process_heredoc_exit(t_cmd, i, prev_infd);
	}
	return (0);
}

// Fonction pour gérer les redirections de sortie (fichiers)
int ft_handle_outfile(t_list *shell, t_commande *t_cmd, t_all *all, int i, int *prev_outfd)
{
	if (*prev_outfd != -1)
		close(*prev_outfd);
...
int ft_handle_append(t_list *shell, t_commande *t_cmd, t_all *all, int i, int *prev_outfd)
{
	if (*prev_outfd != -1)
		close(*prev_outfd);
